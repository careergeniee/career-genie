...
export const DailyTaskReminder = () => {
    ...
    if (!today && career && !promptDismissed) {
        return (
            <div className="mx-4 mt-4 lg:mx-8 rounded-2xl border border-primary/30 bg-primary/5 p-3.5 flex items-center gap-3">
                <Sparkles className="w-5 h-5 text-primary shrink-0" />
                <p className="text-sm flex-1">
                    Your senior instructor has today's task ready for you.
                </p>
                <button
                    onClick={() => navigate("/dashboard/instructor")}
                    className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-gradient-gold text-primary-foreground shrink-0"
                >
                    Get task <ArrowRight className="w-3.5 h-3.5" />
                </button>
